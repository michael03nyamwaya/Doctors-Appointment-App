...
const RelatedDoc = ({ docId, speciality }) => {
    const { doctors } = useContext(AppContext)  // Accessing doctors from context using useContext
    const [relDoc, setRelDoc] = useState([])
    const navigate = useNavigate()  // Removed 'useCallback' and used navigate directly

    useEffect(() => {
        if (doctors.length > 0 && speciality) {  // Fixed typo in 'length'
            ...
            <div className='grid grid-cols-auto w-full gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {
                    relDoc.slice(0, 5).map((item, index) => (
                        <div key={index} onClick={() => { navigate(`/appointments/${item._id}`); scrollTo(0, 0) }} className='border border-blue-200 rounded-xl overflow-hidden hover:translate-y-[-10px] transition-all duration-500 cursor-pointer'>
